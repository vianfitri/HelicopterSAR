...
# Inisialisasi Data Block Modbus (panjang 200 register)
...
slave_store = ModbusSlaveContext(
    di=discrete_inputs,    # Function Code 2 (Discrete Inputs / X)
    co=coils,              # Function Code 1 (Coils / Y)
    hr=holding_registers,  # Function Code 3 (Holding Registers / V)
)

# Sesuaikan parameter 'slaves' untuk versi pymodbus baru
...
def simulate_plc_data():
    ...
    while True:
        try:
            # 1. Update Discrete Inputs X7 - X10 (FC 2 / Discrete Inputs)
            x_values = [
            ...
            # Menggunakan parameter 'function_code' pengganti 'fx'
            slave_store.setValues(
                function_code=2, address=7, values=x_values
            )

            # 2. Update Output Lampu Y8 - Y11 (FC 1 / Coils)
            y_values = [
            ...
            slave_store.setValues(function_code=1, address=8, values=y_values)

            # 3. Update Register V32, V34, V102 (FC 3 / Holding Registers)
            ...
            slave_store.setValues(
                function_code=3, address=32, values=[rpm]
            )  # V32 (RPM)
            slave_store.setValues(
                function_code=3, address=34, values=[speed2]
            )  # V34 (SPEED2)
            slave_store.setValues(
                function_code=3, address=102, values=[hoist_load]
            )  # V102 (Hoist Load)
