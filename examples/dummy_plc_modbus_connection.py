    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # 1. Baca Discrete Inputs X7 - X10 (FC 02)
                res_x = self.client.read_discrete_inputs(address=7, count=4)

                # 2. Baca Coils Y8 - Y11 (FC 01)
                res_y = self.client.read_coils(address=8, count=4)

                # 3. Baca Holding Registers V32 s.d V102 (FC 03)
                res_v = self.client.read_holding_registers(address=32, count=71)

                if not (res_x.isError() or res_y.isError() or res_v.isError()):
                    # --- INVERSI LOGIKA NORMALLY CLOSED (NC) ---
                    # Bit False pada PLC/Modbus = Aktif/ON di Visual GUI
                    x_vals_nc = [not bit for bit in res_x.bits[:4]]
                    y_vals_nc = [not bit for bit in res_y.bits[:4]]

                    # Ambil Status NC dari Y10 (Hoist Up) dan Y11 (Hoist Down)
                    y10_hoist_up_active = y_vals_nc[2]    # True jika Aktif (Modbus bit False)
                    y11_hoist_down_active = y_vals_nc[3]  # True jika Aktif (Modbus bit False)

                    # --- PENGHITUNGAN PANJANG HOIST ---
                    if y11_hoist_down_active:
                        self.hoist_length += HOIST_SPEED_M_PER_SEC * dt
                    elif y10_hoist_up_active:
                        self.hoist_length -= HOIST_SPEED_M_PER_SEC * dt
                    ...
                    wx.CallAfter(
                        ...
                        x_vals_nc, y_vals_nc, v32_val, v34_val, v102_val, self.hoist_length
                    )
...
class MainWindow(wx.Frame):
    def __init__(self):
        super().__init__(parent=None, title="Haiwell PLC AT16S0R Monitor (NC Mode)", size=(620, 560))
    ...
    def init_ui(self):
        ...
        # 2. Status Input X7 - X10 (NC)
        input_box = wx.StaticBox(panel, label="Status Limit Switch - NC (Discrete Input X)")
        ...
        # 3. Status Output Y8 - Y11 (NC)
        output_box = wx.StaticBox(panel, label="Status Lampu Output - NC (Output Y)")
    ...
    def update_ui_data(self, x_vals_nc, y_vals_nc, v32, v34, v102, hoist_length):
        if not self.is_connected:
            return

        # Update Indikator X (Input NC)
        inputs = [self.lbl_x7, self.lbl_x8, self.lbl_x9, self.lbl_x10]
        for lbl, is_active in zip(inputs, x_vals_nc):
            lbl.SetLabel("ON" if is_active else "OFF")
            lbl.SetBackgroundColour(wx.Colour(46, 204, 113) if is_active else wx.Colour(210, 210, 210))

        # Update Indikator Y (Output NC)
        outputs = [self.lbl_y8, self.lbl_y9, self.lbl_y10, self.lbl_y11]
        for lbl, is_active in zip(outputs, y_vals_nc):
            lbl.SetLabel("ON" if is_active else "OFF")
            lbl.SetBackgroundColour(wx.Colour(241, 196, 15) if is_active else wx.Colour(210, 210, 210))
    ...
    def reset_ui_elements(self):
        """Mereset semua tampilan UI ke kondisi mati / 0 saat koneksi putus."""
        all_indicators = [
